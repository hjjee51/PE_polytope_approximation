...
class PE_analysis:
	...
	def __init__(self, P_reg, P_in, NPA_level=2, Poly_in=None):
		self.P_reg = P_reg # Expected P from characterisation stage (should be in Q, i.e., regularised) - conditional form: P_reg[a,b,x,y] = P(ab|xy)
		self.P_in = P_in # Input distribution - P_in[x,y] = p(x,y)
		self.E_reg = p2E_bi(self.P_reg) # P_reg in the correlator form
		self.rel_CHSH_ineq = [] # The relevant CHSH inequality
		self.q_rel_CHSH_ineq = [] # The relevant quantum CHSH inequality (with Tsirelson's bound)
		self.find_rel_CHSH_ineq()
	...
	def find_rel_CHSH_ineq(self):
		'''
		Find the relevant CHSH inequality which P_reg violates.
		There is always one.
		'''
		for ineq in CHSH_ineqs:
			value = np.einsum('ij,ij->', ineq, self.E_reg)
			if value<0:
				self.rel_CHSH_ineq = ineq.copy()
				break
		if self.rel_CHSH_ineq==[]:
			print("The distribution does not violate any CHSH inequality.")
			return
		self.q_rel_CHSH_ineq = self.rel_CHSH_ineq.copy()
		self.q_rel_CHSH_ineq[0][0] = 2*np.sqrt(2)

	def add_Poly_option(self, option, Poly):
		'''
		Function to add a new polytope option apart from {'NS','NS+CHSH', 'NearV', 'MaxGP'}
		'''
	...
	def generate_Poly(self, option, niter=1, cutoff=20, prob=None, fix_input=False):
		'''
		...
		if option == "NS":
			...
			self.Poly[option] = [self.generate_Poly_NS_plus_CHSH()]
		elif option == "NearV":
	...
	def generate_Poly_NS_plus_CHSH(self):
		'''
		Generate the polytope for option 'NS+CHSH'.
		The generated polytope includes NS distributions 
		satisfying the Tsirelson's bound for the relevant CHSH inequality.
		'''
		ineqs = positivity_NS[:] + [self.q_rel_CHSH_ineq[:]]
		
		H_rep = []
		for ineq in ineqs:
		    H_rep.append(ineq.flatten())

		mat = cdd.Matrix(H_rep, number_type='float')
		mat.rep_type = cdd.RepType.INEQUALITY
		poly = cdd.Polyhedron(mat)
		V_rep = np.array(poly.get_generators())
		return V_rep

	def generate_Poly_with_NearV(self, niter=1, cutoff=20, prob=None):
		'''
		Generate poltyopes following 'NearV' algorithm.
		'''
		print("=================== NearV algorithm starts ===================")
		if self.Poly["NS+CHSH"] == []: 
			print("Please generate 'NS+CHSH' polytope first.")
			return

		GivenH = positivity_NS[:] + [self.q_rel_CHSH_ineq]
		Poly_i = self.Poly["NS+CHSH"][0].copy()
		Poly_list = []
		for i in range(niter):
			...
			ext_list = []
			for ext in Poly_i:
				p = E2p_bi(np.reshape(ext, (3,3)))
				try: is_quantum = check_quantum_NPA(np.round(p, decimals=5), level=3)
				except: 
			...
			m = min(cutoff, len(norm_lsit))
			ind_nearest = np.argsort(norm_list)[:m]
			...
			ext = np.reshape(ext_list[ind_nearest[ext_ind]], (3,3))
	...
	def generate_Poly_with_NearV(self, niter=1, cutoff=20, prob=None):
		...
		for i in range(niter):
			...
			while len(ind_nearest)<m:
				...
				p = E2p_bi(np.reshape(Poly_i[ind_nearest_b[j]], (3,3)))
				try: is_quantum = check_quantum_NPA(np.round(p, decimals=5), level=3)
				except: 
					print("MOSEK failed -  too close to the quantum set.")
					is_quantum = True
			...
			print("Chosen index:",ext_ind)
			ext = np.reshape(Poly_i[ind_nearest[ext_ind]], (3,3))
	...
	def generate_Poly_with_MaxGP(self, niter=1, fix_input=False):
		...
		for i in range(niter):
			if fix_input == False:
				...
				x,y = fix_input
			GP, GP_optE = GP_bi_2out_fullS_GivenH(self.E_reg, GivenH, xbar=x, ybar=y)
			...
			for j in range(len(GP_optE)):
				...
				try: is_quantum = check_quantum_NPA(np.round(E2p_bi(GP_optE[j]), decimals=5), level=3)
				except: 
					print("MOSEK failed -  too close to the quantum set.")
					continue
				if is_quantum: 
			...
			Poly_i = self.generate_V_from_GivenH(GivenH)
			Poly_list.append(Poly_i)
			print("---------------------")

		return Poly_list

	def get_optF(self, option, n_list=None, power_list=None, first_power=0.0001, n_points=101, MOSEK=True):
		'''
		...
		Input - option: polytope option such as 'NearV' or 'NS'
				...
				first_power: when 'power_list' is not given, the first power in the created power_list
				n_points: when 'power_list' is not given, the number of points in the created power_list
		...
		for n in n_list:
			...
			for power in tqdm(self.power_list):
				if MOSEK:
					logPR, optF = PEF_opt_bi_2out(self.P_reg, self.P_in, polytope, self.Poly_in, power)
				else:
					logPR, optF = PEF_opt_bi_2out_SQP(self.P_reg, self.P_in, polytope, self.Poly_in, power)
	...
	def generate_q_BI_from_E(self, E):
		...
		'''
		nearest_Q = find_nearest_Q(E, NPA_level=self.NPA_level)
		bell_coeff = np.round(E - nearest_Q, decimals=6)
		bell_coeff = bell_coeff*10
		max_Q_value, max_Q_point = bi_max_Q_violation(bell_coeff, NPA_level=self.NPA_level)

		new_ineq = -bell_coeff.copy()
		new_ineq[0,0] = np.round(max_Q_value, decimals=6)+10**(-6)
		return new_ineq
	...
	def randomness_accumulation(self, N_raw, eps_sec, option, n_list=None, show_result=False, save=False, title=None):
		...
		for n in n_list:
			...
			for i, power in enumerate(self.power_list):
				...
				# netlogPR_list.append(self.calculate_netlogPR(N_raw, optF, beta, N, eps_sec))
	...
	def calculate_extEntR(self, N_raw, optF, power, N, eps):
		...
		'''
		return np.einsum('ijlm,ijlm->', N_raw, np.log2(optF))/power/N + np.log2(eps/(1+power))/power/N + np.log2(power*eps/(1+power))/N
